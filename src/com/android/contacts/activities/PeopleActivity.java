...
import com.android.contacts.list.FastSearchManager;
...
public class PeopleActivity extends ContactsActivity implements
	...
	private final ContactDetailLoaderFragmentListener mContactDetailLoaderFragmentListener = new ContactDetailLoaderFragmentListener();
	...
	private FastSearchManager mFastSearchManager;
	private OnMenuItemVisibleChangedListener mMenuVisibleChangedListener;
	 ...
	 * 
	 * However, there are special fragments which may not be in the layout, so
	 * we have to do the initialization here. The target fragments are: -
	 * {@link ContactDetailFragment} and {@link ContactDetailUpdatesFragment}:
	 * They may not be in the layout depending on the configuration. (i.e.
	 * portrait) - {@link ContactsUnavailableFragment}: We always create it at
	 * runtime.
	 */
	@Override
	...
	private void createViewsAndFragments(Bundle savedState) {
		...
		final boolean isUsingTwoPanes = PhoneCapabilityTester
				.isUsingTwoPanes(this);
		if (isUsingTwoPanes) {
		...
		if (isUsingTwoPanes) {
			...
			// Only favorites tab with two panes has a separate frequent
			// fragment
			...
			mContactDetailLayoutController = new ContactDetailLayoutController(
					this, savedState, getFragmentManager(),
					mContactDetailsView,
		...
		if (PhoneCapabilityTester.isUsingTwoPanesInFavorites(this)) {
			...
			// For 2-pane in All and Groups but not in Favorites fragment, show
			// the chevron
		...
		// Wang: fast search init
		mFastSearchManager = FastSearchManager.getInstance(PeopleActivity.this);
		mFastSearchManager.setSearchPad(findViewById(R.id.fast_search_inputpad));
		mFastSearchManager.bindActionBarAdapter(mActionBarAdapter);
		mFastSearchManager.bindFragment(mAllFragment);
	...
	protected void onStart() {
		if (!mFragmentInitialized) {
			...
			// TODO: Clean this up. Perhaps change {@link
			// ContactListFilterController} to not be a
			// singleton?
			mContactListFilterController.setContactListFilter(
					mAllFragment.getFilter(), true);
			mContactListFilterController.checkFilterValidity(true);
			mCurrentFilterIsValid = true;
		}
		//Wang:
		mFastSearchManager.onStart();
		super.onStart();
	}
	...
	private void configureFragments(boolean fromRequest) {
		if (fromRequest) {
			...
			case ContactsRequest.ACTION_VIEW_CONTACT:
				// We redirect this intent to the detail activity on 1-pane,
				// so we don't get
	...
	private void setupGroupDetailFragment(Uri groupUri) {
		...
		if (mGroupDetailFragment != null
				&& mGroupDetailFragment.getGroupUri() != null
				&& !UriUtils.areEqual(mGroupDetailFragment.getGroupUri(),
						groupUri)) {
	...
	public void onAction(int action) {
		...
		case ActionBarAdapter.Listener.Action.CHANGE_SEARCH_QUERY:
			...
			break;
			/*Wang:*/
		case ActionBarAdapter.Listener.Action.STOP_FAST_SEARCH_MODE:
			onFastSearchStopped();
			updateFragmentsVisibility();
			invalidateOptionsMenu();
			break;
		...
		if (mContactsUnavailableFragment != null) {
			...
			case TabState.GROUPS:
				mContactsUnavailableFragment.setMessageText(R.string.noGroups,
						areGroupWritableAccountsAvailable() ? -1
								: R.string.noAccounts);
				break;
			case TabState.ALL:
				mContactsUnavailableFragment.setMessageText(
		...
		private Fragment getFragment(int position) {
			if (mTabPagerAdapterSearchMode) {
				...
				if (position == TabState.FAVORITES) {
					...
					//Wang:
					mFastSearchManager.bindAdapter(mAllFragment.getAdapter());
	...
	/**Wang:*/
	private void onFastSearchStopped(){
		mAllFragment.onFastSearchStopped();
		mAllFragment.setVisibleScrollbarEnabled(!mAllFragment.isSearchMode());
	}
	...
	private void configureContactListFragment() {
		...
		mAllFragment
				.setVerticalScrollbarPosition(useTwoPane ? View.SCROLLBAR_POSITION_LEFT
	...
	private final class ContactBrowserActionListener implements
		...
		public void onCallContactAction(Uri contactUri) {
			PhoneNumberInteraction.startInteractionForPhoneCall(
					PeopleActivity.this, contactUri);
		}

		@Override
		public void onSmsContactAction(Uri contactUri) {
			PhoneNumberInteraction.startInteractionForTextMessage(
					PeopleActivity.this, contactUri);
		}
	...
	private class ContactDetailLoaderFragmentListener implements
			ContactLoaderFragmentListener {
	...
	public class ContactDetailFragmentListener implements
		...
		public void onCreateRawContactRequested(
			...
			Intent serviceIntent = ContactSaveService
					.createNewRawContactIntent(PeopleActivity.this, values,
							account, PeopleActivity.class, Intent.ACTION_VIEW);
	...
	private final class StrequentContactListFragmentListener implements
		...
		public void onContactSelected(Uri contactUri, Rect targetRect) {
			if (PhoneCapabilityTester.isUsingTwoPanes(PeopleActivity.this)) {
				QuickContact.showQuickContact(PeopleActivity.this, targetRect,
						contactUri, 0, null);
			} else {
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		if (DEBUG_TRANSITIONS && mContactDetailLoaderFragment != null) {
			final MenuItem toggleSocial = menu.add(mContactDetailLoaderFragment
...
//		makeMenuItemVisible(menu, R.id.menu_search, showMiscOptions);
        ...
        //Wang: 
		if(mMenuVisibleChangedListener != null) mMenuVisibleChangedListener.onMenuItemVisibleChanged(fastSearchMenu);
		return true;
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.menu_settings: {
			...
			// as there is only one section right now, make sure it is
			// selected
			// on small screens, this also hides the section selector
			// Due to b/5045558, this code unfortunately only works properly
			// on phones
		...
		case R.id.menu_add_contact: {
			final Intent intent = new Intent(Intent.ACTION_INSERT,
					Contacts.CONTENT_URI);
			// On 2-pane UI, we can let the editor activity finish itself
			// and return
			...
			if (PhoneCapabilityTester.isUsingTwoPanes(this)) {
				...
				// Otherwise, on 1-pane UI, we need the editor to launch the
				// view contact
				// intent itself.
				startActivity(intent);
			}
		...
		/*Wang: */
		case R.id.menu_fastSearch:{
			mFastSearchManager.toggleSearchPad();
			return true;
		}
		}
		return false;
	 ...
	 * @author Wang
	 * @param titleResId
	 *            resource id of title text. -1 will display default title
	 *            (contacts)
	 * @return
	 * */
	private void displayTitleBarWithoutTabs(final int titleResId) {
		...
		if(mFastSearchManager != null) mFastSearchManager.hideSearchPad();
		bar.setTitle(textId);
	}
	...
	private void createLocalGroup(String groupName) {
		AccountWithDataSet dataSet = new AccountWithDataSet("local", "local",
				null);
		Intent saveIntent = ContactSaveService.createNewGroupIntent(this,
				dataSet, groupName, new long[0], this.getClass(),
	 ...
	 * Wang
	 * */
	public interface OnMenuItemVisibleChangedListener{
	 ...
	 * Wang
	 * */
	public void setOnMenuItemVisibleChangedListener(OnMenuItemVisibleChangedListener listener){
		mMenuVisibleChangedListener = listener;
	}
